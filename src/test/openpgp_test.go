...
func TestFingerprint(t *testing.T) {
	pubkey, _ := openpgp.ReadArmoredKeyRing(strings.NewReader(testPublicKey))
	fmt.Println(hex.EncodeToString(pubkey[0].PrimaryKey.Fingerprint[0:20]))
	buffer := bytes.NewBuffer(nil)
	pubkey[0].Serialize(buffer)
	bufferToHash := bytes.NewBuffer(nil)
	bufferToHash.Write([]byte{0x99})
	bufferToHash.Write([]byte{byte(buffer.Len() >> 8), byte(buffer.Len())})
	bufferToHash.Write(buffer.Bytes())
	hash := sha1.Sum(bufferToHash.Bytes())
	hashBytes := hash[0:]
	fmt.Print(hex.EncodeToString(hashBytes))
	t.Error()
}
